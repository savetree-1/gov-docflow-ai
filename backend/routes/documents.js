...
async function processDocumentWithAI(documentId, filePath, mimeType) {
  try {
    ...
    /****** Notify WebSocket Which will tell that AI Processing is Started ******/
    websocketService.notifyAIStatus(documentId, 'processing', {
      message: 'Analyzing document with AI...',
      stage: 'text_extraction'
    });
    
    /****** STEP 1: Extract text which includes PDF -> pdf-parse, if < 100 chars -> OCR, Image -> OCR ******/
    ...
    if (!documentText || documentText.length < 50) {
      ...
      /****** For scanned/image documents with no extractable text we'll create a basic summary ******/
    ...
    /****** Notification for Text extraction complete ******/
    websocketService.notifyAIStatus(documentId, 'processing', {
    ...
    /****** STEP 3: Get routing suggestions under department assignment  ******/
    ...
    /****** Trying multiple matching strategies to find department ******/
    ...
    /****** Applying routing suggestion which department admin can confirm or edit ******/
    if (suggestedDept) {
      ...
      /****** Converting confidence from decimal to percentage, example 0.99 to 99% ******/
      ...
      document.routingConfirmed = false; /****** Letting this to be confirm and review by admin ******/
      ...
      document.suggestedDepartment = routingSuggestion.primaryDepartment;
      document.routingReason = routingSuggestion.reasoning;
      /****** Converting confidence from decimal to percentage ******/
      const confidence = routingSuggestion.confidence || 0.85;
      document.routingConfidence = confidence < 1 ? Math.round(confidence * 100) : confidence;
      document.routingConfirmed = false; // Let admin review and confirm
      console.log(`Department not found, using initial department as fallback (awaiting confirmation)`);
    }

    /****** STEP 5: Save AI results to database with handling new generateSummary format ******/
    ...
    /****** Handling new deadline format from Gemini ******/
    ...
    /****** Notifying WebSocket that the AI Processing Complete ******/
    websocketService.notifyAIStatus(documentId, 'completed', {
    ...
    /****** Notifying the document's subscribers about the update ******/
...
/****** Post Endpoint to Upload new document ******/
router.post('/upload', authMiddleware, upload.single('file'), async (req, res) => {
  try {
    ...
    /****** Generating the reference number for the document ******/
    ...
    /****** Triggering AI processing in background ******/
...
/****** Get Endpoint to access all documents with filters ******/
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (search) {
      /****** Preserving role-based filtering when searching ******/
...
/****** Get Endpoint to get document by ID ******/
...
/****** Put Endpoint to Perform action on document which includes approve, reject or forward ******/
router.put('/:id/action', authMiddleware, async (req, res) => {
  try {
    ...
    /****** Checking if user has the follwing permissions permission: 
            View, Download, Print, Comment - any logged in user can do this one
    ...
    ******/
    ...
    /****** Updating document status based on action ******/
    if (action === 'Approve') {
    ...
    /****** Logging critical actions to blockchain which are immutable audit trail  ******/
    if (['Approve', 'Reject', 'Forward'].includes(action)) {
      try {
        ...
        /****** Continue the task without failing the action ******/
      }
    ...
    /****** Logging audit with appropriate action type ******/
...
/****** Post Endpoint for Confirming the AI routing suggestion ******/
router.post('/:id/confirm-routing', authMiddleware, async (req, res) => {
  try {
    ...
    /****** Checking permission - uploader, department admin, or super admin can confirm routing ******/
    const isUploader = document.uploadedBy.toString() === req.user.userId;
    const isSuperAdmin = req.user.role === 'SUPER_ADMIN';
    
    /****** Handling both populated and non-populated department fields ******/
    ...
    if (confirmed && !modifiedDepartment) {
      ...
      /****** Escaping special regex characters in department name ******/
    ...
    /****** Logging the routing confirmation to blockchain ******/
    if (finalDepartment) {
      try {
        const User = require('../models/User');
    ...
    /****** Sending email notifications to department admin and officers ******/
    if (finalDepartment) {
      try {
        const User = require('../models/User');
        const Notification = require('../models/Notification');
        
        /****** Get all users in the department which includes admins and officers ******/
        ...
        for (const user of departmentUsers) {
          ...
          /****** Sending email if user has registered email ******/
          if (user.email) {
        ...
        /****** Ensuring that we Don't fail the request if email fails ******/
      }
    }
...
/****** Get Endpoint to Verify document on blockchain ******/
...
/****** Deete endpoint to Soft delete document having access to SUPER_ADMIN only ******/
router.delete('/:id', authMiddleware, roleMiddleware('SUPER_ADMIN'), async (req, res) => {
  try {
    ...
    /****** Soft deleting - mark as deleted which ensures that file stays on disk for recovery ******/
...
/****** Endpoint for Restoring deleted document for SUPER_ADMIN only ******/
...
/****** Endpoint to Permanent delete having access to SUPER_ADMIN only - for archived/deleted documents ******/
...
/******Endpoint to  Get document statistics in the dashboard ******/
...
/****** Endpoint to Download/Preview document file which is a public endpoint ******/
router.get('/:id/download', async (req, res) => {
  try {
    ...
    // /****** Setting proper headers for PDF preview ******
