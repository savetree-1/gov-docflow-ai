...
// Geting audit logs for specific document
...
// Geting audit logs for specific user
...
// Exporting audit logs (CSV format)
router.get('/export/csv', authMiddleware, roleMiddleware('SUPER_ADMIN', 'AUDITOR'), async (req, res) => {
  try {
    ...
    const logs = await AuditLog.find(query)
      ...
      .populate('targetDocument', 'title referenceNumber')
      .sort({ timestamp: -1 })
      .limit(10000); // Limiting the export to 10k records
