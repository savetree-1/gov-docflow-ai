...
/****** Main text extraction service Module which handles both PDFs and images
        1. Pipeline: PDF → pdf-parse → if text < 100 chars → OCR fallback
        2. Image → direct OCR
******/
...
async function extractText(filePath, fileType) {
  try {
    ...
    if (fileType === 'application/pdf') {
      /****** Trying PDF text extraction first ******/
      text = await extractFromPDF(filePath);
      
      /****** If PDF has very little text, it might be scanned then using OCR ******/
      if (text.length < 100) {
...
async function extractFromPDF(filePath) {
  try {
    ...
    return ''; /****** Returning empty to trigger OCR fallback ******/
  }
}
...
/****** Clean extracted text in which we'll remove excessive whitespace and fix encoding issues ******/
