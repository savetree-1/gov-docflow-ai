...
const SuperAdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [deptsRes, usersRes, docsRes, allDeptsRes] = await Promise.all([
        departmentAPI.getStats(),
        userAPI.getStats(),
        documentAPI.getAll({ limit: 50, includeDeleted: false }),
        departmentAPI.getAll({ includeAll: true })
      ]);
      ...
      if (deptsRes.data.success && usersRes.data.success) {
        setMetrics({
          ...
          documentsProcessed: docsRes.data?.total || 0
        });
      }
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      const response = await analyticsAPI.getDashboardAnalytics(selectedTimeRange);
      
      if (response.data.success) {
        const data = response.data.data;
        setDocumentsOverTime(data.documentsOverTime || []);
        setDepartmentPerformance(data.departmentPerformance || []);
        setStatusDistribution(data.statusDistribution || []);
        setProcessingTrends(data.processingTrends || []);
      }
      ...
      <div className="dashboard-content">
        ...
        <div className="dashboard-header">
          <div className="header-content">
            <h1>
              Super Admin <span className="dashboard-highlight">Dashboard</span>
            </h1>
        ...
        >
          ...
          >
            ...
            >
              Pending (
              {allDepartments.filter((d) => d.status === "Pending").length})
            </button>
            <button
            ...
            >
              Approved (
              {allDepartments.filter((d) => d.status === "Approved").length})
            </button>
            <button
            ...
            >
              Rejected (
              {allDepartments.filter((d) => d.status === "Rejected").length})
            </button>
          </div>
            ...
            <div>
              ...
              >
                {showDeleted
                  ...
                  : `View Deleted (${deletedDocuments.length})`}
              </button>
            </div>
          </div>
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      These documents are soft-deleted and hidden from all
                      users. You can restore them or permanently delete them.
                    </p>
                  </div>
                </div>
              </div>
